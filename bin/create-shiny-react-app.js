...
const EXAMPLES = [
  ...
  },
  {
    id: "6-dashboard",
    name: "Dashboard",
    description: "Full dashboard with charts and tables",
  },
  {
    id: "7-chat",
    name: "AI Chat",
...
function updatePackageJson(targetDir, appName) {
  ...
  if (fs.existsSync(packageJsonPath)) {
    ...
    if (packageJson.dependencies && packageJson.dependencies["shiny-react"]) {
      packageJson.dependencies["shiny-react"] = "^0.0.4";
    }
...
async function main() {
  ...
  // Locate shiny-react package examples
  let shinyReactPath;
  try {
    ...
    try {
      const resolvedPath = require.resolve("shiny-react", { paths: [cliPackageDir] });
      ...
      const directPath = path.join(cliPackageDir, "node_modules", "shiny-react");
    ...
    console.error("Error: shiny-react package not found.");
    console.error("Please ensure shiny-react is installed.");
    console.error("Run: npm install -g create-shiny-react-app");
    process.exit(1);
  }

  const examplesDir = path.join(shinyReactPath, "examples");
  if (!fs.existsSync(examplesDir)) {
    console.error("Error: Examples directory not found in shiny-react package.");
    process.exit(1);
  }
  ...
  try {
    ...
    const templateDir = path.join(examplesDir, selectedExample.id);
    ...
    // Update package.json with the new app name and proper shiny-react dependency
    updatePackageJson(targetDir, appName);

    // Copy CLAUDE.md template if requested
    if (shouldIncludeClaude) {
      const claudeTemplatePath = path.join(examplesDir, "CLAUDE.md.template");
